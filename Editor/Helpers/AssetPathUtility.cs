...
namespace MCPForUnity.Editor.Helpers
{
    ...
    public static class AssetPathUtility
    {
        ...
        /// <summary>
        /// Gets the Yoji Unity MCP package root path.
        ...
        /// <summary>
        /// Reads and parses the package.json file for Yoji Unity MCP.
        ...
        /// <summary>
        /// Gets the package source for the bundled MCP server (used with uvx --from).
        /// Checks for EditorPrefs override first, then uses the package-local Server~ directory.
        /// No remote package fallback is used in this fork so installs do not silently run upstream code.
        /// </summary>
        /// <returns>Package source string for uvx --from argument, or empty if unavailable.</returns>
        public static string GetMcpServerPackageSource()
        {
            ...
            string bundledServer = GetBundledServerPath();
            if (!string.IsNullOrEmpty(bundledServer))
            {
                return bundledServer;
            }

            McpLog.Warn("Bundled MCP server not found. Expected Server~/pyproject.toml inside the installed package.");
            return string.Empty;
        }

        private static string GetResolvedPackageRootPath()
        {
            var packageInfo = PackageInfo.FindForAssembly(typeof(AssetPathUtility).Assembly);
            if (packageInfo != null && !string.IsNullOrEmpty(packageInfo.resolvedPath))
            {
        ...
        private static string GetBundledServerPath()
        {
            string packageRoot = GetResolvedPackageRootPath();
            if (string.IsNullOrEmpty(packageRoot))
            {
                return null;
            }
        ...
        /// Non-local paths (URLs and package references) are returned unchanged.
        /// </summary>
        internal static string ResolveLocalServerPath(string path)
        {
            ...
            // Skip non-local paths (git URLs and package names).
            ...
            // If it looks like a package reference (no path separators), skip
            ...
            // Check if a server subdirectory contains pyproject.toml.
            foreach (string child in new[] { "Server~", "Server" })
            {
                string serverSubDir = System.IO.Path.Combine(checkPath, child);
        ...
        public static (string uvxPath, string fromUrl, string packageName) GetUvxCommandParts()
        {
            ...
            string packageName = "yoji-unity-mcp";
        ...
        /// Uses an explicit override or the bundled Server~ package source.
        /// Centralizes the prerelease logic to avoid duplication between HTTP and stdio transports.
        /// Priority: explicit fromUrl override > bundled Server~ path.
        ...
        public static string GetBetaServerFromArgs(string gitUrlOverride, string packageSource, bool quoteFromPath = false)
        {
            ...
            // Standard mode: use bundled or explicitly configured server package source.
        ...
        /// Runs a lightweight probe (uvx --offline ... yoji-unity-mcp --help) with a 3-second timeout
        ...
        private static bool RunOfflineProbe()
        {
            try
            {
                ...
                string probeArgs = string.IsNullOrEmpty(fromArgs)
                    ? "--offline yoji-unity-mcp --help"
                    : $"--offline {fromArgs} yoji-unity-mcp --help";
