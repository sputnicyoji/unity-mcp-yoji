...
{
    /// <summary>
    /// Manages dynamic port allocation and persistent storage for Yoji Unity MCP
    /// </summary>
    public static class PortManager
    {
        ...
        /// <summary>
        /// Check if a port is currently being used by Yoji Unity MCP
        /// This helps avoid unnecessary port changes when Unity itself is using the port
        /// </summary>
        /// <param name="port">Port to check</param>
        /// <returns>True if port appears to be used by Yoji Unity MCP</returns>
        public static bool IsPortUsedByMCPForUnity(int port)
        {
            try
            {
                // Try to make a quick connection to see if it's an Yoji Unity MCP server
                using var client = new TcpClient();
                var connectTask = client.ConnectAsync(IPAddress.Loopback, port);
                if (connectTask.Wait(100)) // 100ms timeout
                {
                    // If connection succeeded, it's likely the Yoji Unity MCP server
