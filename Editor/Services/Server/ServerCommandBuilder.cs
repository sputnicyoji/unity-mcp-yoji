...
namespace MCPForUnity.Editor.Services.Server
{
    ...
    public class ServerCommandBuilder : IServerCommandBuilder
    {
        ...
        public bool TryBuildCommand(out string fileName, out string arguments, out string displayCommand, out string error)
        {
            ...
            if (!useHttpTransport)
            {
                error = "HTTP transport is disabled. Enable it in the Yoji Unity MCP window first.";
                return false;
            }
            ...
            // Use bundled or explicitly configured server package source
            string fromArgs = AssetPathUtility.GetBetaServerFromArgs(quoteFromPath: true);
            if (string.IsNullOrEmpty(fromArgs))
            {
                error = "Bundled Python server was not found. Expected Server~/pyproject.toml inside the installed package.";
                return false;
            }
