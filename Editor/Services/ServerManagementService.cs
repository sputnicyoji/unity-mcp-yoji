    ...
    public class ServerManagementService : IServerManagementService
    {
        ...
        public bool StartLocalHttpServer(bool quiet = false)
        {
                    ...
                    if (remaining.Count > 0)
                    {
                        if (!quiet)
                        {
                            EditorUtility.DisplayDialog(
                                ...
                                "Yoji Unity MCP will not terminate unrelated processes. Stop the owning process manually or change the HTTP URL.",
                                "OK");
                        }
                        return false;
                    }
            ...
            try
            {
                ...
                string effectiveLog = launchLog ?? Path.Combine(Path.GetTempPath(), "yoji-unity-mcp-server-launch.log");
                ...
                // Privacy hardening. The bundled server has no external reporting sender, and these
                // env vars also protect explicit server-source overrides.
                startInfo.EnvironmentVariables["DISABLE_TELEMETRY"] = "1";
                startInfo.EnvironmentVariables["UNITY_MCP_DISABLE_TELEMETRY"] = "1";
                startInfo.EnvironmentVariables["MCP_DISABLE_TELEMETRY"] = "1";
        ...
        private bool StopLocalHttpServerInternal(bool quiet, int? portOverride = null, bool allowNonLocalUrl = false)
        {
            ...
            try
            {
                ...
                // - Only terminate processes that look like the MCP server (uv/uvx/python running yoji-unity-mcp).
                ...
                if (TryGetStoredLocalServerPid(port, out int storedPid))
                {
                    if (pids.Contains(storedPid))
                    {
                        ...
                        {
                            // Never kill Unity/Hub.
                            // Note: "yoji-unity-mcp" includes "unity", so detect MCP indicators first.
                            bool storedMentionsMcp = storedArgsLowerNow.Contains("yoji-unity-mcp")
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    allowKill = storedArgsLowerNow.Contains("uvicorn")
                                                ...
                                                || storedArgsLowerNow.Contains("mcpforunity")
                                                || storedArgsLowerNow.Contains("yoji-unity-mcp")
                                                || storedArgsLowerNow.Contains("mcp_for_unity")
                ...
                foreach (var pid in pids)
                {
                    ...
                    if (!LooksLikeMcpServerProcess(pid))
                    {
                        if (!quiet)
                        {
                            McpLog.Warn($"Refusing to stop port {port}: owning PID {pid} does not look like yoji-unity-mcp.");
                        }
                        continue;
                    }
