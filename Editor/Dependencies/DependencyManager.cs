...
namespace MCPForUnity.Editor.Dependencies
{
    ...
    public static class DependencyManager
    {
        ...
        private static void GenerateRecommendations(DependencyCheckResult result, IPlatformDetector detector)
        {
            ...
            if (missing.Count == 0)
            {
                result.RecommendedActions.Add("All dependencies are available. You can start using Yoji Unity MCP.");
                return;
            }
            ...
            {
                result.RecommendedActions.Add("Use the Setup Window (Window > Yoji Unity MCP > Local Setup Window) for guided installation.");
            }
        }
    }
}
