...
async def server_lifespan(server: FastMCP) -> AsyncIterator[dict[str, Any]]:
    ...
    logger.info(f"Yoji Unity MCP Server v{_server_version} starting up")
    ...
    try:
        ...
        logger.info("Yoji Unity MCP Server shut down")
...
def create_mcp_server(project_scoped_tools: bool) -> FastMCP:
    ...
    async def health_http(_: Request) -> JSONResponse:
        return JSONResponse({
            ...
            "message": "Yoji Unity MCP server is running"
        })
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Yoji Unity MCP Server",
