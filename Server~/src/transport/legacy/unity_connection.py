...
logger = logging.getLogger("yoji-unity-mcp-server")
            ...
            try:
                ...
                try:
                    ...
                    if 'FRAMING=1' in text:
                        ...
                        logger.debug(
                            'Yoji Unity MCP handshake received: FRAMING=1 (strict)')
                    else:
                        if require_framing:
                            # Best-effort plain-text advisory for legacy peers
                            with contextlib.suppress(Exception):
                                self.sock.sendall(
                                    b'Yoji Unity MCP requires FRAMING=1\n')
                            raise ConnectionError(
                                f'Yoji Unity MCP requires FRAMING=1, got: {text!r}')
                        else:
                            self.use_framing = False
                            logger.warning(
                                'Yoji Unity MCP handshake missing FRAMING=1; proceeding in legacy mode by configuration')
                finally:
...
class UnityConnectionPool:
    ...
    def _resolve_instance_id(self, instance_identifier: str | None, instances: list[UnityInstanceInfo]) -> UnityInstanceInfo:
        ...
        if not instances:
            raise ConnectionError(
                "No Unity Editor instances found. Please ensure Unity is running with Yoji Unity MCP bridge."
            )
